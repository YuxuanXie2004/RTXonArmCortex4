...
#define PENDSVSET   (1 << 28)  // Bit 28 sets PendSV pending
extern U32 systick_active;
extern U32 continue_schedule;
extern U32 resume_schedule;
/* USER CODE END PD */
...
void SysTick_Handler(void)
{
  ...
  // if (systick_active == 1){
    ...
    for (int i = 1; i < MAX_TASKS; i++) {
      if (task_list[i].state == DORMANT) continue;
    ...
    // if (running_stop==1 || sleeping_stop==1) {
    //     if (running_stop && current_tid != TID_NULL) {
    //         halt_TID = current_tid;
    //     }
    //      _ICSR |= PENDSVSET;
    //      __asm("MOV r0, #17");
